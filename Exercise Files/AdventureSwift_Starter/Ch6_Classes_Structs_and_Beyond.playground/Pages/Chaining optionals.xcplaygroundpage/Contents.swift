...
var rareDagger = Item(description: "pfft", previousOwner: nil)
var daggerOwner = Owner(name: "yonker")
rareDagger.previousOwner = daggerOwner
rareDagger.previousOwner?.name = "fartface"

if let owner = rareDagger.previousOwner?.name {
    print("used to be owned by \(owner)")
} else {
    print("idk")
}
...
if let gemstoneob = questDirectory["Fetch Gemstones"]?["Objective"] {
    print(gemstoneob)
}
